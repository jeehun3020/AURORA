...
## 발견 6 — 항행창은 넓어졌지만 최적점은 이동하지 않았다

NSIDC 1979~2025년 **관측만** 사용한다. ERA5도 위험지수도 쓰지 않으므로 모델 가정에 의존하지 않는다.

### 6-1. 개빙기는 크게 늘었다 (종료일 지연이 주도)

| 해역 | 개빙기 길이 79~83 → 21~25 | 47년 변화 | 종료일 이동 | p |
|---|---|---|---|---|
| 카라해 | 51일 → **121일** | +101.7일 | 09-30 → **11-05** | <10⁻¹⁰ |
| 추크치해 | 34일 → **93일** | +92.6일 | 09-30 → 10-31 | <10⁻⁶ |
| 랍테프해 | 22일 → **87일** | +72.6일 | 09-24 → 10-16 | <10⁻⁶ |
| 동시베리아해 | 32일 → **53일** | +40.0일 | 10-01 → 10-10 | 0.038 |

확장은 주로 **종료일 지연**에서 온다. 시즌이 가을 쪽으로 늘어난 것이지 양쪽으로 균등하게 벌어진 것이 아니다.

### 6-2. 그러나 최적 시점은 그대로다

| 해역 | 최소빙일 79~83 → 21~25 | 이동 | 판정 |
|---|---|---|---|
| 추크치해 | 09-12 → 09-18 | +5.8일 | n.s. |
| 동시베리아해 | 09-13 → 09-16 | +2.8일 | n.s. |
| 랍테프해 | 09-15 → 09-12 | −2.8일 | n.s. |
| 카라해 | 09-21 → 09-12 | −9.4일 | * (p=0.029) |

**4개 중 3개에서 최소빙일 이동이 통계적으로 불확실하다.** 유일하게 유의한 카라해는 오히려 **앞당겨졌다.**

> **의미**: 항행 가능 기간이 두 배 이상 늘어난 47년 동안에도 "가장 좋은 시점"은 9월 중순에 머물렀다. 시즌이 길어졌다고 최적기가 뒤로 밀린 것이 아니다. 따라서 §발견 3의 9월 중순 권고는 장기 관측으로도 지지된다.

### 6-3. 다만 최적일의 연간 변동이 크다 — 달력만으로는 부족한 이유

최근 10년(2016~2025) 최소빙일 분포:

| 해역 | 중앙값 | 10~90 퍼센타일 | 표준편차 |
|---|---|---|---|
| 추크치해 | 09-18 | 09-15 ~ 09-23 | 3.3일 |
| 동시베리아해 | 09-19 | 09-11 ~ 09-25 | 8.2일 |
| 랍테프해 | 09-18 | 08-30 ~ 10-01 | 13.1일 |
| **카라해** | 09-12 | **08-22 ~ 09-28** | **16.0일** |

카라해는 최적일이 연도별로 **한 달 이상** 흔들린다.

> **이것이 발견 5와 연결되는 지점이다.** 기후학적 최적기는 9월 중순으로 안정적이지만, 개별 연도의 최적일은 최대 16일 표준편차로 흩어진다. 그래서 달력(기후값) 규칙은 이론적 최대이득의 7~25%밖에 얻지 못하고, 예측 기반 규칙이 60% 이상을 얻는다. **"평균 최적일"은 계획의 출발점일 뿐 결정 근거가 될 수 없다.**

---

## 발견 7 — 합성지수는 해빙지수의 재포장이 아니다

현재 지수는 해빙이 평균수준의 51.0%, 일별변동의 81.7%를 차지한다(§4 모델 구조 감사). 화물선 AIS 확보가 무산되어 합성의 **추가가치**는 검증할 수 없다. 그러나 **"검증할 수 없다"와 "차이가 없다"는 다르다.** 행동자료 없이도 두 지수가 실제로 다른 결정을 내리는지는 답할 수 있다.

| 비교 | 결과 |
|---|---|
| 값 상관 (Spearman) | **0.887** — 함께 움직이지만 동일하지 않음 |
| 구간×월 위험순위 변동 | **13/16 칸**에서 순위 변동 (추크치해 10월은 6단계) |
| **3일 출항결정 일치율** | **57.5%** (4,800건 중 2,038건 불일치) |
| **7일 출항결정 일치율** | **61.3%** (4,640건 중 1,798건 불일치) |

**두 지수는 출항 결정의 약 40%에서 다른 답을 낸다.** 예측오차가 전혀 없는 완전예지 조건에서도 그렇다. 즉 불일치는 잡음이 아니라 지수 구조의 차이에서 온다.

> **정확한 서술**: 합성지수는 해빙지수를 이름만 바꾼 것이 아니다. 실질적으로 다른 결정을 내린다. **다만 어느 쪽 결정이 옳은지는 화물선 AIS 없이 판정할 수 없다.**

**하면 안 되는 주장**: 합성이 해빙단독보다 낫다. (정답 라벨이 없다. 합성지수를 기준으로 평가하면 합성이 이기는 것이 당연하므로 그 비교는 순환논증이다.)
